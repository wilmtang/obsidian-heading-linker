...
Consider renaming the setting to "Basename" vs "Full vault path", or add a source-aware copy mode that computes a relative path from the active/source note to the target file. If keeping current behavior, add tests covering duplicate basenames so the tradeoff is explicit.
...
- Unit tests cover source-aware rewrites, stable IDs, active-editor line rewrites, duplicate heading counting, and line-ending preservation.
...
- Escaped markdown destination parsing, especially `>` in headings.
